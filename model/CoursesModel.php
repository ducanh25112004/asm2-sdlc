...
function updateCoursesById(
    ...
    $db = connectionDb();
    $checkUpdate = false;
    $sql = "UPDATE `courses` SET `name` = :nameCourses, `slug` = :slug, `status` = :statusCourses, `department_id` = :idDepartment,`description` = :descriptionCourses,`updated_at` = :updated_at WHERE `id` = :id AND `deleted_at` IS NULL";
    $updateTime = date('Y-m-d H:i:s');
    $stmt = $db->prepare($sql);
    if ($stmt) {
        $stmt->bindParam(':nameCourses', $name, PDO::PARAM_STR);
        $stmt->bindParam(':slug', $slug, PDO::PARAM_STR);
        $stmt->bindParam(':statusCourses', $status, PDO::PARAM_INT);
        $stmt->bindParam(':idDepartment', $idDepartment, PDO::PARAM_INT);
        $stmt->bindParam(':descriptionCourses', $description, PDO::PARAM_STR);
...
function getDetailCoursesById($id = 0)
{
    $db = connectionDb();
    ...
    if ($stmt) {
        ...
        if ($stmt->execute()) {
            if ($stmt->rowCount() > 0) {
                $infoCourses = $stmt->fetch(PDO::FETCH_ASSOC);
            }
        }
    }
    disconnectionDb($db);
...
function deleteCoursesById($id = 0)
{
...
function getAllDataCourses($keyword = null)
{
    ...
    $sql = "SELECT departments.name AS department_name, courses.*
            ...
            AND courses.`deleted_at` IS NULL AND departments.`deleted_at` IS NULL ";
    $stmt = $db->prepare($sql);
    $key = "%{$keyword}%";
    $data = [];
    if ($stmt) {
...
function getAllDataCoursesByPage($keyword = null, $start = 0, $limit = LIMIT_ITEM_PAGE)
{
    ...
    $sql = "SELECT departments.name AS department_name, courses.*
            FROM departments
            INNER JOIN courses ON departments.id = courses.department_id 
            WHERE (courses.`name` LIKE :keywordName OR courses.`slug` LIKE :keywordSlug) 
            AND courses.`deleted_at` IS NULL AND departments.`deleted_at` IS NULL 
...
function insertCourses(
    ...
    $db = connectionDb();
    $flagInsert = false;
    $sqlInsert = "INSERT INTO `courses`(`name`, `slug`, `status`, `description`, `department_id`, `created_at`) VALUES(:nameCourses, :slug, :statusCourses, :descriptionCourses, :idDepartment, :created_at)";
