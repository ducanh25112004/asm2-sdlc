...
function handleUpdate()
{
    if (isset($_POST['btnUpdate'])) {
        ...
        // check du lieu
        $_SESSION['error_courses'] = [];

        if (empty($name)) {
            $_SESSION['error_courses']['name'] = 'Enter courses, please!';
        } else {
            $_SESSION['error_courses']['name'] = null;
        }
        if (empty($slug)) {
            $_SESSION['error_courses']['slug'] = "Enter courses instructor, please!";
        } else {
            $_SESSION['error_courses']['slug'] = null;
        }

        $checkError = false;
        foreach ($_SESSION['error_courses'] as $error) {
            if (!empty($error)) {
                $checkError = true;
                break;
            }
        ...
        if ($checkError) {
            ...
            if (isset($_SESSION['error_courses'])) {
                unset($_SESSION['error_courses']);
            }
            //$slug = slugify($slug);
...
function handleAdd()
{
    ...
    if (isset($_POST['btnSave'])) {
        ...
        if (empty($slug)) {
            $_SESSION['error_courses']['slug'] = "Enter name, please!";
        } else {
            $_SESSION['error_courses']['slug'] = null;
        }

        if (
            !empty($_SESSION['error_courses']['name'])
            ||
            !empty($_SESSION['error_courses']['slug'])
        ) {
            ...
            $insert = insertCourses($name, $slug, $status, $description, $idDepartment);
            if ($insert) {
                header("Location:index.php?c=courses&state=success");
            } else {
...
function edit()
{
    ...
    $infoDetail = getDetailCoursesById($id);
    if (!empty($infoDetail)) {
